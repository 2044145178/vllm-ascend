...
| `cpu_binding_process_fraction`      | list | `None`  | Restricts the current worker to the relative `[start, end]` slice of its NPU-local main CPU pool. Use disjoint fractions for multiple engine processes sharing one NPU, for example `[0.0, 0.625]` and `[0.625, 1.0]`. |
| `decode_fia_static_kv_len`          | int  | `0`     | Pads single-request FIA decode to a fixed KV length and updates a graph-stable mask instead of rebuilding FIA graph tasks. It must not exceed the paged-KV capacity; requests longer than the configured value fall back to dynamic FIA updates. |
