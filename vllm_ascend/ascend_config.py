...
class AscendConfig:
    ...
    def __init__(self, vllm_config: "VllmConfig"):
        ...
        self.decode_fia_static_kv_len = int(additional_config.get("decode_fia_static_kv_len", 0))
        if self.decode_fia_static_kv_len < 0:
            raise ValueError("decode_fia_static_kv_len must be non-negative")
        ...
        if cpu_binding_fraction is None:
            ...
            if not isinstance(cpu_binding_fraction, (list, tuple)) or len(cpu_binding_fraction) != 2:
                raise ValueError("cpu_binding_process_fraction must be a two-element list [start, end]")
            start, end = (float(value) for value in cpu_binding_fraction)
            if not 0.0 <= start < end <= 1.0:
                raise ValueError(
                    f"cpu_binding_process_fraction must satisfy 0 <= start < end <= 1, got [{start}, {end}]"
                )
